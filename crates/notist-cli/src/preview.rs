...
const PAGE: &str = r#"<!doctype html><html><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>Notist</title><style>body{font:16px system-ui;margin:32px auto;max-width:960px;padding:0 20px;line-height:1.6}nav{border-bottom:1px solid #ddd;padding:12px 0}main{white-space:pre-wrap}notist-error,pre#error{color:#a21d32}canvas,svg{max-width:100%}</style><nav><strong>Notist</strong> <select aria-label="Module" id="modules"></select></nav><pre id="error"></pre><main></main><script type="module">
import {mount} from '/renderer.js';
let last='',busy=false;
const select=document.querySelector('select');
select.onchange=()=>{last='';refresh()};
async function refresh(){if(busy)return;busy=true;try{const response=await fetch('/content?module='+encodeURIComponent(select.value));const data=await response.json();if(!response.ok)throw Error(data.error);if(!select.options.length){for(const name of data.modules)select.add(new Option(name,name));select.value=data.entry;}const next=JSON.stringify(data);if(next!==last){await mount(document.querySelector('main'),data.content,data.components,location.origin+'/');last=next;}document.querySelector('#error').textContent=data.warnings.join('\n');}catch(e){document.querySelector('#error').textContent=e.message}finally{busy=false}}
            ...
            match notist_next::package::load(&root) {
                ...
                Ok(mut loaded) => {
                    if url.starts_with("/content?") {
                        ...
                        (200,"application/json",serde_json::json!({"entry":entry,"modules":loaded.runtime.sources.keys().collect::<Vec<_>>(),"content":result.content.to_json(),"warnings":result.warnings,"components":loaded.components}).to_string().into_bytes())
