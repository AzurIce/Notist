...
在仓库根运行 `just web-build` 构建浏览器求值器，`just web-fixtures` 同步示例，`just web-compare` 对同一 package 请求比较 native 与 WASM 的最终 Content 和诊断。需要本地服务时，手动运行 `just web-serve`，访问 http://127.0.0.1:8000/app/。
