...
const { analyze } = createRequire(import.meta.url)('./pkg-node/notist_next_web.js');
const run = flag => JSON.parse(execFileSync('cargo', ['run', '-q', '-j4', '-p', 'notist-next', '--', 'crates-next/notist-next/examples/demo', flag], { cwd: root, encoding: 'utf8' }));
