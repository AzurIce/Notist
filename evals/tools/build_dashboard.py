                ...
                for c in content:
                    ...
                    if t == "text":
                        ...
                        items.append({
                            ...
                            "id": str(c.get("id", "")),
                            "name": c.get("name", "?"),
                            "args": json.dumps(c.get("arguments"), ensure_ascii=False)[:2000],
                        })
            if role == "toolResult":
                result_text = ""
                ...
                items.append({
                    "kind": "toolresult",
                    "id": str(m.get("toolCallId", "")),
                    "name": m.get("toolName", "?"),
                    "isError": bool(m.get("isError")),
                    "text": result_text[:6000],
                })
...
.pre{background:#0c0e14;border:1px solid var(--line);border-radius:6px;padding:8px;max-height:260px;overflow:auto;white-space:pre-wrap;font-size:12px;margin:6px 0}
...
<script>window.EVAL_SESSIONS=window.EVAL_SESSIONS||{};window.EVAL_ANSWERS=window.EVAL_ANSWERS||{};
window.onerror=function(m,s,l){const el=document.getElementById("sess");if(el)el.innerHTML="⚠ 脚本错误: "+esc(String(m));return false;};</script>
...
function renderTrace(msgs){
  ...
  const blocks=[];
  let thinkingHtml="",textHtml="",pendingTools=[];
  function flushAssistant(){
    if(thinkingHtml===""&&textHtml===""&&pendingTools.length===0)return;
    if(thinkingHtml||textHtml)
      blocks.push(`<div class="msg assistant"><div class="role">助手</div>`+
        (thinkingHtml?`<details><summary>思考过程</summary>${thinkingHtml}</details>`:"")+
        textHtml+`</div>`);
    pendingTools=[];
    thinkingHtml="";textHtml="";
  }
  for(const m of msgs){
    if(m.usage){totalIn+=m.usage.in||0;totalOut+=m.usage.out||0;}
    if(m.role==="user"){
      flushAssistant();
      for(const it of m.items)if(it.kind==="text")
        blocks.push(`<div class="msg user"><div class="role">用户任务</div><div>${esc(it.text)}</div></div>`);
      continue;
    }
    if(m.role==="assistant"){
      for(const it of m.items){
        if(it.kind==="thinking")thinkingHtml+=`<div style="white-space:pre-wrap;color:var(--dim)">${esc(it.text)}</div>`;
        else if(it.kind==="text")textHtml+=`<div>${esc(it.text)}</div>`;
        else if(it.kind==="toolcall"){
          flushAssistant();
          calls++;
          blocks.push(`<div class="msg tool"><div class="role">工具 · ${esc(it.name)}</div>`+
            `<details><summary>参数</summary><pre class="pre">${esc(it.args||"")}</pre></details>`+
            `<div class="pending" id="pending-${esc(it.id)}" style="color:var(--dim);font-size:12px">运行中…</div></div>`);
          byId[it.id]=blocks.length-1;
        }
      }
      continue;
    }
    if(m.role==="toolResult"){
      for(const it of m.items){
        if(it.kind!=="toolresult")continue;
        const idx=byId[it.id];
        const resHtml=`<details ${it.isError?"open":""}><summary class="${it.isError?"fail":""}">工具结果${it.isError?"（错误）":""}</summary>`+
          `<pre class="pre out">${esc(it.text||"(空)")}</pre></details>`;
        if(idx!==undefined&&!String(blocks[idx]).includes("id=\"done\""))
          blocks[idx]=blocks[idx].replace(`<div class="pending" id="pending-${esc(it.id)}" style="color:var(--dim);font-size:12px">运行中…</div>`,resHtml);
        else
          blocks.push(`<div class="msg toolResult"><div class="role">工具结果${it.isError?"（错误）":""}</div><pre class="pre out">${esc(it.text||"(空)")}</pre></div>`);
      }
      continue;
    }
  }
  flushAssistant();
  const header=`<div style="color:var(--dim);font-size:12px;margin:6px 0">${msgs.length} 条消息 · ${calls} 次工具调用 · Σ in ${tok(totalIn)} / out ${tok(totalOut)}</div>`;
  return header+blocks.join("")||"(空会话)";
}
...
function loadSession(runId,key){
  ...
  s.onload=()=>{
    ...
    if(!msgs||!msgs.length){$("#sess").innerHTML="<span style='color:var(--warn)'>该格没有会话记录——任务超时被终止或模型未产生落盘会话（如 TD1 的 skill 条件这一格）。</span>";return;}
    $("#sess").innerHTML=renderTrace(msgs);
  };
