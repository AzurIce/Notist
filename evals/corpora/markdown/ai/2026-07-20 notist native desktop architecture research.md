...
| 方案 | 原生程度 | 编辑器适配 | WebView 扩展 | 跨平台 | 主要代价 | 结论 |
| --- | --- | --- | --- | --- | --- | --- |
| Windows WinUI + macOS AppKit/SwiftUI + Linux GTK | 最高 | 各平台可深度优化 | 各平台原生 WebView | 高，但三套实现 | UI 工作量约三倍，行为一致性难 | 团队足够大时才选 |
| Qt 6 Widgets/QML + Qt WebEngine | 高 | 成熟文本、dock、窗口能力 | 很成熟 | 高 | C++/Rust FFI、包体、构建与许可评估 | 稳定性优先的强备选 |
| Floem + editor-core + Wry | 高 | 候选中最贴近 Rust 编辑器 | 可通过 Wry 嵌入 | Windows/macOS/Linux | Floem pre-1.0，Wry/Wayland 集成需验证 | 首选技术验证 |
| Slint + Wry | 高 | 通用控件好，编辑器需较多自研 | raw-window-handle 可连接 Wry | Windows/macOS/Linux | 文本编辑器成本、Slint 许可、Wayland 集成 | UI 稳定性优先的备选 |
| GPUI + Wry | 高 | Zed 证明其适合编辑器 | 理论可嵌入 | 当前三平台可用 | pre-1.0、文档少、紧随 Zed 演进 | 适合愿意跟进上游的团队 |
| egui/Iced/Xilem + Wry | 高 | 通用 UI 可用，专业编辑器需自研 | 可做但集成需验证 | 高 | IME、复杂文本、可访问性和编辑体验工作量大 | 不作为首选 |
| Tauri 2 + CodeMirror 6 | 核心 native，UI 为 Web | 非常成熟 | 天然 | 高 | 主要 UI 不是 native，系统 WebView 差异 | 最快交付的保底方案 |
| Electron + Monaco/CodeMirror | UI 为 Web | 成熟 | 天然 | 高 | 资源占用和 Chromium 包体 | 不符合当前目标 |

<a id="native-ui-框架分析"></a>
## Native UI 框架分析
...
<a id="为什么不能每个-element-一个-webview"></a>
...
<a id="何时考虑-cef-或-qt-webengine"></a>
...
当前 `Element::Custom { name, body, block }` 和 `notist-html` 的安全 fallback 是一个良好起点。建议把 renderer registry 明确放在语义模型之后：
...
<a id="为什么不默认支持-native-dynamic-library"></a>
...
<a id="spike-anative-editor最高优先级"></a>
...
这个方案不会把未来锁死：native editor 可以替换为 CodeMirror，Floem shell 可以替换为 Slint/Qt，系统 WebView 可以替换为 CEF，但 `WorkspaceSnapshot`、`EditorPort`、`RenderNode`、WIT plugin API 和 capability model 可以保持稳定。

<a id="外部资料"></a>
## 外部资料

- Wry README：cross-platform engine、child webview、Linux GTK/Wayland 注意事项和 feature flags。https://github.com/tauri-apps/wry
- Slint README 与 Rust crate features：stable 1.x、renderer、accessibility、raw-window-handle、Winit/Qt backend 和许可。https://github.com/slint-ui/slint
- Floem README 与 editor-core：跨平台 native Rust UI、成熟度声明和编辑器基础模型。https://github.com/lapce/floem
- GPUI README：平台 backend、pre-1.0 状态和编辑器导向架构。https://github.com/zed-industries/zed/tree/main/crates/gpui
- Iced README。https://github.com/iced-rs/iced
- Xilem README。https://github.com/linebender/xilem
- egui README。https://github.com/emilk/egui
- Wasmtime README 与 guide：WASI、Component Model、安全和资源配置。https://github.com/bytecodealliance/wasmtime 和 https://docs.wasmtime.dev/
- CodeMirror 6 documentation。https://codemirror.net/docs/
- Microsoft WebView2 distribution guide。https://learn.microsoft.com/en-us/microsoft-edge/webview2/concepts/distribution
- WebKitGTK API reference。https://webkitgtk.org/reference/webkit2gtk/stable/
- Tauri process model，可作为系统 WebView 多进程和 IPC 设计的参考。https://v2.tauri.app/concept/process-model/
