...
<a id="typst-的-element-function-与语法糖设计"></a>
# Typst 的 Element Function 与语法糖设计

```not
#callout(kind="note", title=[文档定位])[
本文是 Typst 机制调查，Typst 示例不等于 Notist 语法。文末的 Notist 建议已按当前实现校正；完整现行规则以 #<vault::cheatsheet>、#<vault::functions> 和 #<vault::grammar> 为准。
]
```
...
<a id="直接对应-element-的语法糖"></a>
## 直接对应 Element 的语法糖

| 表面语法 | 底层 Element | 大致等价写法 |
|---|---|---|
| `Hello` | `TextElem` | `#text("Hello")` |
| `*strong*` | `StrongElem` | `#strong[strong]` |
| `_emphasis_` | `EmphElem` | `#emph[emphasis]` |
| `` `raw` `` | `RawElem` | `#raw("raw")` |
| `https://typst.app` | `LinkElem` | `#link("https://typst.app")` |
| `= Title` | `HeadingElem` | `#heading(level: 1)[Title]` |
| `\` | `LinebreakElem` | `#linebreak()` |
| 空行 | `ParbreakElem` | `#parbreak()` |
| `'` / `"` | `SmartQuoteElem` | `#smartquote(...)` |
| `$x$` | `EquationElem` | `#math.equation[...]` |
...
<a id="不是普通-element-function-的语法"></a>
...
<a id="importinclude-与-context"></a>
...
<a id="typst-语法的四种类型"></a>
## Typst 语法的四种类型

<a id="element-sugar"></a>
### Element Sugar
...
- Typst Element 定义：`crates/typst-library/src/model/` 与 `crates/typst-library/src/text/`
