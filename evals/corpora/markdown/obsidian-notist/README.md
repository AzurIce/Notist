...
#[
  ...
  目前想到的方案是，基于 Obsidian 的插件，将 Notist 的能力接入其中。即让 Obsidian 的 GUI 壳承载 Notist 的能力。

  初始调查由 kimi/k3 整理在了 #<vault::ai::2026-08-24 obsidian plugin feasibility research> 中。

  #<vault::ai::2026-08-24 obsidian plugin feasibility research/Obsidian 双链与图谱引擎专题> 中提到 Obsidian 自己的双链、图谱等引擎只对 Markdown 生效且无法干预。因此我们大概需要自己实现一整套基于 Notist 且与 Obsidian 自身引擎独立的系统。
...
- [lsp](lsp.md)：.not 编辑器接入 notist LSP 的方案（桌面 spawn `notist lsp` 走 stdio、自写薄 JSON-RPC client、文档同步与 CM 集成点、降级策略）。
- #<vault::ai::2026-08-24 obsidian plugin feasibility research>：本模块的可行性调研与决策记录；其中 #<vault::ai::2026-08-24 obsidian plugin feasibility research/Obsidian 双链与图谱引擎专题> 详查官方引擎的实现与扩展性——结论是主走自建索引与渲染（语义权威本就在 notist-service），注入 `resolvedLinks` 白嫖官方 graph 至多作可降级的可选增强。
- #<vault::ai::2026-08-17 web ui obsidian-like editor research>：CM6 选型、.not 语法接入两路径、LSP-over-WS。
- #<vault::ai::2026-08-18 notist plugin system design>：含 Obsidian 插件机制调研。
- 先例：Excalidraw（Obsidian 内独立世界）、obsidian-mermaid-view（registerExtensions + 三模式 view）、Code Workbench（外部 LSP client）。
