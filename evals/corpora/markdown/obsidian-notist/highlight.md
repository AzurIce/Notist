...
- `git -C ~/Files/tree-sitter-notist pull`；
- `nix develop -c tree-sitter build --wasm`（其 devShell 提供 tree-sitter CLI；wasi-sdk 已由 CLI 自动下载到 `~/.cache/tree-sitter`，无需 emscripten）；
- `tree-sitter-notist.wasm` 与 `queries/highlights.scm` 拷入本插件 `assets/`，并把 revision 记进 `assets/UPSTREAM.txt`；
...
- #<vault::ai::2026-08-24 obsidian-notist editor cm6 migration>：编辑器内核与样式约定，本文的挂载点。
- #<vault::ai::2026-08-24 obsidian plugin feasibility research>：壳/语义分层原则（语义组件不 import obsidian API）——解析层设计同样遵守。
- `zed-notist` 仓库（`~/Files/zed-notist`）：`grammars/notist.wasm`、`languages/notist/*.scm` 资产来源。
