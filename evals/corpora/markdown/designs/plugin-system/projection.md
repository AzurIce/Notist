...
```text
成型后的森林
  → reduce（target registry）     // 插件/未知 call → html::*
  → html:: 森林                   // target 数据节点；core 保留原生内置分支
  → serialize                     // 标签出名字、属性出参数、children 递归
```
...
- target registry 按名字分发投影 handler；
...
HtmlContribution {
  element: String
  trusted: Bool
  web-component: Option<WebComponentDecl>
}
...
- 宿主由 manifest 合成投影 handler：输出 custom element，并把标量字段转义为 `data-*` 属性；字段与 body 值保持与其他 HTML 投影相同的转义规则。
- JS module 与 CSS 由 CLI build / preview 复制到 `_notist/plugins/<package>/` 并注入页面 head。
...
- 已裁定：fallback 以 call 名出标签（`<{package}-{element}>`），永远 total。
- 待定：trusted 边界暂缓——验证核心设计期间 serializer 不做词表白名单，`html::*` 全部放行；正式化前必须收口（危险标签与事件属性的 gate，`trusted` 标记的语义）。
- 待定：插件的"原生 HTML 投影贡献"（tag + class、无 JS module）是否开放。
- 待定：HTML 之外 native target 的 render node 契约。
