...
负数是一元 `-` 运算符（字面量本身无符号）。科学计数法暂缓。

String 字面量四种形态（继承旧字符串字面量设计的历史决定）：
...
- 形参必须声明类型（与命名函数一致，[call-model](call-model.md)）；返回类型由 body 推断；
...
├─ T?                    # 可空（type system）
└─ fn(形参表) -> R       # 形参：名字: 类型，带 = 表示可省略（call model）
```

标注位置：`let name: T = ...`、函数形参、函数返回类型、lambda 形参。
...
```text
= Title / == Title        → syntax-sugar heading 节（含默认 id）
```lang ... ```            → syntax-sugar raw 节
---                       → syntax-sugar rule 节
- A / + A                 → syntax-sugar item 节
| a | b | / | - | - |     → syntax-sugar table 节（pipe table）
#<target>                → syntax-sugar ref 节
*text* / _text_ / __text__ / ~~text~~  → syntax-sugar strong/emph/underline/strike 节
@id,#tag,.class,key=value → annotation-syntax 标注
@[...] / @![...]          → annotation-syntax 标注
{...} / #[...]             → markup-surface scope 语法形态
