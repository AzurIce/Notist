...
| scope | 语义角色 | 成型中的存在方式 |
|---|---|---|
| 根 scope | 模块本身；正文是隐式 Content quotation | 输出求值结果 bindings 与 content |
| Content literal `[...]` | Code 中的内容字面量；自带词法边界 | 透明 scope：溶解为内容序列 |
| Code block `{...}` | 多语句局部作用域；块值按 join 规则组合 | 值参与外层插入，不保留为结构节点 |
| 手动 scope `#[...]` | 显式 scope：封装内容、词法边界、可承载 `@id` 与属性 | 透明 scope：溶解，属性/身份转为区间条目 |
| 元素 scope | 构造器产生的内容节点（heading、item、callout 等 body） | 实节点：保留为结构树节点 |
...
- 元素 scope 是实节点，保留为结构树节点；
- 透明 scope（Content literal 与手动 scope）溶解进节点序列参与统一分组——scope 的内容可以与外部文字连排，溶解后其内容可能分属多个父节点，树无法把它装成单个节点；
- 溶解时 scope 的属性与身份以区间条目进入属性表，覆盖它内容所在的区域，可以跨越段落边界（[property-table](property-table.md) 属性表、[property-table](property-table.md)）。
...
| 输入 | 预期 | 测试 |
|---|---|---|
| `#let accent = "violet"` 后 `= #accent` | 根 binding 对后续正文可见，并进入求值结果 bindings | `let_bindings_flow_into_later_markup` |
...
- 待定：递归绑定（`let rec`）的形态与规则；未引入前 `let` 右侧不可见正在声明的名字。
