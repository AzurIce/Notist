...
本文是集合类型的目标设计草案；当前 surface 尚未实现。进入实现切片后，本文与 #<vault::designs::language::type-system>、#<vault::designs::language::code-grammar> 同步成为现行规范。
]
```
...
- `Array<T>` / `Dict<K, V>` 与对应 first-class immutable Value；
- union type `T | U` 与内部 `Never`。

不进入本文：

- 可变集合与原地 mutation（Typst `push` 会原地修改并返回 `none`；Notist 保持纯求值与不可变 Value）；
- method/field access 语法（`.at`、`.keys` 等）；
- `for` 循环；
- 用户可写的 generic function；
...
```text
Array<T>     T 为元素类型
Dict<K, V>   K 为 key 类型，V 为 value 类型
```
...
(1, 2, 3)                    // Array<Int>
(1,)                         // 单元素 Array<Int>
()                           // 空 Array；未标注时成员类型为 Never
("theme": "dark", "n": 2)   // Dict<String, Int | String>
(:)                          // 空 Dict；未标注时 K/V 为 Never/Never
```

- `(expr)` 仍是 grouping；顶层 comma 选择 Array，顶层 `expr: expr` 选择 Dict，二者不能混写；
- `(x: Int) => ...` 仍是 lambda：parser 用 `=>` lookahead 区分参数表与 Dict literal；
...
Value::Dict { entries: Vec<(Value, Value)>, ty: Type }
```
...
| 输入 | 诊断 |
|---|---|
| `(1, "a", 2)` | 无；类型为 `Array<Int | String>` |
| `(1, 2) == (1, 2)` | `Bool` |
| `("a": 1, "a": 2)` | 无；值为 `Dict<String, Int>`，`"a"` → 2 |
| `(1.0: "x")` | Float 不能作为 Dict key |
...
| source | 预期 | 测试名 |
|---|---|---|
| `(1, 2, 3)` | `Array<Int>`，elements 1/2/3 | `array_literal_infers_int` |
| `(1, "a")` | `Array<Int | String>` | `array_literal_joins_union` |
| `#let xs: Array<Int> = ()` | 空 Array 接受 expected type | `empty_array_accepts_expected_type` |
