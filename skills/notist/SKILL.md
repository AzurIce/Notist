...
- do not page further to re-derive it, and use
`--exclude-scope vault::ai` to separate current specs from dated research
(the `by scope:` line buckets matching modules for you).
Search answers "does this exist at all" from its coverage totals (`matched N modules / M units`) without paging to completion; multi-term queries require all terms somewhere in one MODULE, not in one paragraph. Every hit names its enclosing heading (the `§section` marker); outline rows carry their subtree byte span, so `read path#heading` and citations can target exact ranges without a second probe. Read one whole section by title: `notist read "path.not#Heading text"` returns the section subtree, not just the heading line. Module ids containing spaces are printed quoted - copy them verbatim. Lexical search returns a small page grouped by source by default; use `--group section` for section diversity or `--group match` to locate every occurrence. Multi-term lookup matches all terms by default; pass `--any` only for deliberate broad recall. Narrow result sets with the repeatable `--scope MODULE` and `--exclude-scope MODULE` filters. Search excerpts select candidates; do not treat them as complete evidence.
