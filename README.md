...
LSP 支持 `.not` / `.notc` 的语法诊断、嵌套 section 大纲、局部 let 和顶层导入的定义跳转与悬停、基础补全，以及 Item label 引用的定义跳转和缺失、歧义诊断，支持 UTF-8 / UTF-16 和增量同步。分析层按需解析 Item 引用；`check` 检查求值遇到的目标。Markdown 前端、完整类型分析、daemon、搜索及编辑器自定义预览协议尚未实现。`docs/` 是仓库根 package 的源码目录，可用 `cargo run -p notist-cli -- check .` 校验。

`notist lsp` 从编辑器的 `workspaceFolders`（回退到 `rootUri`，再回退到启动目录）发现 `Notist.toml`，分别建立 package 模块表，并沿相对 manifest 的 path 依赖加载目录外的 package。无需 workspace 配置；发现过程跳过 `.git`、`target`、`node_modules`、`.obsidian` 和 `.direnv`。一个会话共享依赖的文件、AST 和导出索引，未保存源码覆盖磁盘内容。LSP 不加载 WASM 二进制；依赖 WASM 生成的目标无法求值时保留未解析状态，不据此报告 label 缺失或歧义。manifest 按保存后的磁盘内容加载，磁盘变化通过通知或两秒轮询更新；未归属 package 的已打开文件仍有语法能力。嵌套作用域的导入解析尚未实现。
